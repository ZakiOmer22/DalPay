...
class USSDService {
    constructor() {
        this.sessionStore = new USSDSessionStore();
        this.paymentService = new payment_service_1.PaymentService();
        this.taxService = new tax_service_1.TaxService();
    }
    async processRequest(phoneNumber, text, sessionId) {
        ...
        try {
            ...
            logger_1.default.error('USSD state error', { error, session });
            return {
    ...
    async startNewSession(phoneNumber) {
        const phoneHash = (0, encryption_1.hashField)(phoneNumber);
        ...
        const state = { sessionId, userId, phoneNumber, state: 'MAIN' };
    ...
    async handleCheckBalance(session, text) {
        if (text === '0') {
            session.state = 'MAIN';
            this.sessionStore.set(session.sessionId, session);
            const menu = 'Welcome to DalPay USSD Service\n1. Check Balance\n2. Pay Tax\n3. Statement\n4. Exit';
    ...
    async handlePayTaxSelectType(session, text) {
        ...
        const assessmentResults = await database_1.default.query(`SELECT id, amount, tax_type, year, due_date FROM tax_assessments
       WHERE user_id = $1 AND tax_type = $2 AND status IN ('unpaid','partially_paid')
       ORDER BY due_date LIMIT 1`, [session.userId, taxType]);
