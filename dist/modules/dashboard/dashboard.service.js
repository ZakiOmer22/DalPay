...
class DashboardService {
    async getOverview() {
        const [revenueResult, outstandingResult, userResult] = await Promise.all([
            database_1.default.query(`SELECT COALESCE(SUM(amount), 0) as total_revenue
         FROM payments WHERE status = 'confirmed'`),
            database_1.default.query(`SELECT COALESCE(SUM(amount), 0) as total_outstanding
        ...
        const fraudResult = await database_1.default.query(`SELECT COUNT(*) FILTER (WHERE flagged = TRUE) as flagged,
              COUNT(*) as total_analyses
       FROM fraud_analysis`);
        const disputesResult = await database_1.default.query(`SELECT COUNT(*) FILTER (WHERE status = 'pending') as pending_disputes,
    ...
    async getRecentPayments(limit = 10) {
        const result = await database_1.default.query(`SELECT p.id, p.amount, p.status, p.created_at, u.full_name
       FROM payments p
    ...
    async getRecentFraudFlags(limit = 10) {
        const result = await database_1.default.query(`SELECT fa.payment_id, fa.risk_score, fa.reason, fa.created_at, p.amount, u.full_name
