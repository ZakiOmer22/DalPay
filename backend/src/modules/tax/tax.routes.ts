...
router.post('/assessments/generate', authenticate, authorize('admin', 'super_admin'), taxController.generateAssessments);
...
router.get('/disputes/all', authenticate, authorize('admin', 'super_admin'), taxController.getAllDisputes);
router.patch('/disputes/:disputeId', authenticate, authorize('admin', 'super_admin'), taxController.resolveDispute);

router.get('/ledger', authenticate, authorize('admin', 'auditor'), async (req, res, next) => {
  try {
