...
export default function RegisterPage() {
    ...
    useEffect(() => {
        ...
        if (savedData && savedSessionId) {
            try {
                ...
                verificationApi.checkStatus(savedSessionId)
                    .then((statusResponse) => {
                        ...
                        if (status === 'verified') {
                            setError("");
                            setSuccessMessage("Identity verified successfully! You can continue to the next step.");
                            setFormData((prev) => ({ ...prev, agreeToStripeVerification: true }));
                        } else if (status === 'processing') {
                            setSuccessMessage("Stripe is still processing your documents. This page will update automatically.");
                            startPolling(savedSessionId);
                        } else if (status === 'requires_input') {
                            setSuccessMessage("Verification incomplete. Click 'Start Verification' to try again.");
                        } else if (status === 'requires_action') {
                            setError("Verification failed. Stripe needs additional information. Please try again with clear documents.");
            ...
            try {
                ...
                switch (status) {
                    case 'verified':
                        ...
                        break;
                    case 'processing':
                        setSuccessMessage("Stripe is analyzing your documents. This usually takes 1-2 minutes...");
                        break;
                    case 'requires_input':
                    ...
                    case 'requires_action':
                        clearInterval(pollInterval);
                        setIsVerifying(false);
                        setError("Verification failed. Stripe requires additional information. Please try again.");
                        setSuccessMessage("");
    ...
    const handleStripeVerification = async () => {
        ...
        try {
            ...
            const sessionId = response.data.verificationId;
            const verificationUrl = response.data.verificationUrl;

            setStripeVerificationId(sessionId);
            setSuccessMessage("Verification page opened in a new tab. Complete all steps there — this page will update automatically.");
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            setError("");
            setSuccessMessage(response.message || "Account created successfully! Redirecting to your dashboard...");
        ...
        <section className="min-h-screen bg-white dark:bg-[#0A0E1A] pt-28 pb-20 px-4">
            <div className="max-w-3xl mx-auto">
                    ...
                    <div className="flex items-center justify-between relative">
                        ...
                        {stepLabels.map((label, idx) => (
                            <div key={idx} className="relative z-10 flex flex-col items-center">
                                ...
                                <span className="text-xs mt-2 text-gray-600 dark:text-gray-400 hidden sm:block">{label}</span>
                            </div>
                        ))}
                    </div>
                </div>
                ...
                <div className="bg-white dark:bg-[#111627] border border-gray-200 dark:border-gray-700 rounded-2xl p-6 md:p-8 shadow-xl">
                    ...
                    {step === 1 && (
                        <div className="space-y-5 animate-fade-in">
                            ...
                            <div>
                                ...
                                <input type="text" value={formData.nationalId} onChange={(e) => updateField("nationalId", e.target.value)}
                            ...
                            <div>
                                ...
                                <select value={formData.occupation} onChange={(e) => updateField("occupation", e.target.value)}
                                    className="w-full px-4 py-3 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl text-gray-900 dark:text-white focus:outline-none focus:border-[#0F7B8C] focus:ring-2 focus:ring-[#0F7B8C]/20 transition-all">
                                    <option value="">Select</option>
                        ...
                        <div className="space-y-5 animate-fade-in">
                            ...
                            <div>
                                ...
                                <div className="relative">
                                    ...
                                    <input type="tel" value={formData.phoneNumber} onChange={(e) => updateField("phoneNumber", e.target.value)}
                            ...
                            <div>
                                ...
                                <div className="relative">
                                    ...
                                    <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 dark:text-gray-500 hover:text-gray-900 dark:hover:text-white">
                            ...
                            <div>
                                ...
                                <input type="password" value={formData.confirmPassword} onChange={(e) => updateField("confirmPassword", e.target.value)}
                                    className={`w-full px-4 py-3 bg-white dark:bg-gray-800 border rounded-xl text-gray-900 dark:text-white focus:outline-none focus:ring-2 transition-all ${
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                                <div>
                                    ...
                                    <select value={formData.region} onChange={(e) => { updateField("region", e.target.value); updateField("district", ""); }}
                                        className="w-full px-4 py-3 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl text-gray-900 dark:text-white focus:outline-none focus:border-[#0F7B8C] focus:ring-2 focus:ring-[#0F7B8C]/20 transition-all">
                                ...
                                <div>
                                    ...
                                    <select value={formData.district} onChange={(e) => updateField("district", e.target.value)} disabled={!formData.region}
                                        ...
                                        {districts.map((d) => <option key={d} value={d.toLowerCase().replace(/\s+/g, "_")}>{d}</option>)}
                                    </select>
                                </div>
                            </div>
                            <div>
                                ...
                                <textarea value={formData.address} onChange={(e) => updateField("address", e.target.value)}
                                    placeholder="Enter your full street address..." rows={2}
                    ...
                    {step === 3 && (
                        <div className="space-y-5 animate-fade-in">
                            ...
                            <p className="text-sm text-gray-600 dark:text-gray-400">
                                DalPay uses Stripe Identity to securely verify your identity. A new tab will open for you to upload your documents and take a selfie.
                            </p>
                            ...
                            <div className="bg-gray-100/30 dark:bg-gray-800/30 border border-gray-200 dark:border-gray-700 rounded-xl p-6 text-center">
                                {verificationStatus === 'verified' ? (
                                    <>
                                        ...
                                        <h3 className="text-lg font-bold text-gray-900 dark:text-white mb-2">Identity Verified!</h3>
                                        <p className="text-sm text-gray-600 dark:text-gray-400">Your identity has been successfully verified through Stripe. You can proceed.</p>
                                    </>
                                    ...
                                    <>
                                        ...
                                        <h3 className="text-lg font-bold text-gray-900 dark:text-white mb-2">Analyzing Documents...</h3>
                                        <p className="text-sm text-gray-600 dark:text-gray-400">Stripe is verifying your documents against official records. This usually takes 1-2 minutes.</p>
                                    </>
                                    ...
                                    <>
                                        ...
                                        <h3 className="text-lg font-bold text-gray-900 dark:text-white mb-2">Waiting for Documents</h3>
                                        <p className="text-sm text-gray-600 dark:text-gray-400">Please complete the verification form in the opened tab. Upload your ID and take a selfie.</p>
                                        <button onClick={handleStripeVerification} disabled={isVerifying} className="mt-3 text-sm text-[#0F7B8C] dark:text-[#3BA7BC] hover:text-[#0A5D6B] dark:hover:text-[#3BA7BC]/80 font-medium">Reopen verification page</button>
                                    </>
                                    ...
                                    <>
                                        ...
                                        <p className="text-sm text-gray-600 dark:text-gray-400 mb-4">Stripe was unable to verify your identity. This could be due to unclear documents or mismatched information. Please try again with clear photos.</p>
                                        <button onClick={handleStripeVerification} disabled={isVerifying} className="inline-flex items-center gap-2 px-6 py-3 bg-[#0F7B8C] hover:bg-[#3BA7BC] text-white font-semibold rounded-xl transition-all disabled:opacity-50"><ShieldCheck size={18} /> Try Again</button>
                                    </>
                                    ...
                                    <>
                                        ...
                                        <h3 className="text-lg font-bold text-gray-900 dark:text-white mb-2">Verification Canceled</h3>
                                        <p className="text-sm text-gray-600 dark:text-gray-400 mb-4">The verification session was closed before completion. Please try again.</p>
                                        <button onClick={handleStripeVerification} disabled={isVerifying} className="inline-flex items-center gap-2 px-6 py-3 bg-[#0F7B8C] hover:bg-[#3BA7BC] text-white font-semibold rounded-xl transition-all disabled:opacity-50"><ShieldCheck size={18} /> Retry Verification</button>
                                    </>
                                ) : (
                                    <>
                                        ...
                                        <h3 className="text-lg font-bold text-gray-900 dark:text-white mb-2">Verify Your Identity</h3>
                                        <p className="text-sm text-gray-600 dark:text-gray-400 mb-4">Click below to start verification. A new tab will open for Stripe's secure verification page.</p>
                                        <button onClick={handleStripeVerification} disabled={isVerifying} className="inline-flex items-center gap-2 px-6 py-3 bg-[#0F7B8C] hover:bg-[#3BA7BC] text-white font-semibold rounded-xl transition-all disabled:opacity-50">
                                            {isVerifying ? <><Loader2 size={18} className="animate-spin" /> Creating session...</> : <><ShieldCheck size={18} /> Start Verification with Stripe</>}
                                        </button>
                                    </>
                                )}
                            </div>

                            <div><label className="block text-sm font-medium text-gray-900 dark:text-white mb-2">ID Document Type</label><select value={formData.idType} onChange={(e) => updateField("idType", e.target.value)} className="w-full px-4 py-3 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl text-gray-900 dark:text-white focus:outline-none focus:border-[#0F7B8C] focus:ring-2 focus:ring-[#0F7B8C]/20 transition-all">{ID_TYPES.map((t) => <option key={t.value} value={t.value}>{t.label}</option>)}</select></div>
                            <div><label className="block text-sm font-medium text-gray-900 dark:text-white mb-2">Document Number *</label><input type="text" value={formData.idNumber} onChange={(e) => updateField("idNumber", e.target.value)} placeholder="Enter your ID/passport number" className="w-full px-4 py-3 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl text-gray-900 dark:text-white placeholder:text-gray-400 dark:placeholder:text-gray-500 focus:outline-none focus:border-[#0F7B8C] focus:ring-2 focus:ring-[#0F7B8C]/20 transition-all" /></div>
                            {formData.idType === "driving_license" && <div><label className="block text-sm font-medium text-gray-900 dark:text-white mb-2 flex items-center gap-2"><Car size={16} className="text-[#0F7B8C]" /> Driving License Number *</label><input type="text" value={formData.drivingLicenseNumber} onChange={(e) => updateField("drivingLicenseNumber", e.target.value)} placeholder="Enter your driving license number" className="w-full px-4 py-3 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl text-gray-900 dark:text-white placeholder:text-gray-400 dark:placeholder:text-gray-500 focus:outline-none focus:border-[#0F7B8C] focus:ring-2 focus:ring-[#0F7B8C]/20 transition-all" /></div>}
                            <div><label className="block text-sm font-medium text-gray-900 dark:text-white mb-2 flex items-center gap-2"><Home size={16} className="text-[#0F7B8C]" /> Proof of Address Type</label><select value={formData.proofOfAddressType} onChange={(e) => updateField("proofOfAddressType", e.target.value)} className="w-full px-4 py-3 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl text-gray-900 dark:text-white focus:outline-none focus:border-[#0F7B8C] focus:ring-2 focus:ring-[#0F7B8C]/20 transition-all">{PROOF_OF_ADDRESS_TYPES.map((t) => <option key={t.value} value={t.value}>{t.label}</option>)}</select></div>
                        </div>
                    )}
                    ...
                    {step === 4 && (
                        <div className="space-y-5 animate-fade-in">
                            <h2 className="text-xl font-bold text-gray-900 dark:text-white flex items-center gap-2"><FileText size={20} className="text-[#0F7B8C]" /> Agreements & Parental Control</h2>
                            <div className="space-y-3">
                                <label className="flex items-start gap-3 cursor-pointer"><input type="checkbox" checked={formData.agreeToStripeVerification} onChange={(e) => updateField("agreeToStripeVerification", e.target.checked)} className="mt-1 w-4 h-4 rounded border-gray-200 dark:border-gray-700 text-[#0F7B8C] focus:ring-[#0F7B8C]" /><span className="text-sm text-gray-600 dark:text-gray-400">I consent to identity verification through Stripe Identity.</span></label>
                                <label className="flex items-start gap-3 cursor-pointer"><input type="checkbox" checked={formData.agreeToTerms} onChange={(e) => updateField("agreeToTerms", e.target.checked)} className="mt-1 w-4 h-4 rounded border-gray-200 dark:border-gray-700 text-[#0F7B8C] focus:ring-[#0F7B8C]" /><span className="text-sm text-gray-600 dark:text-gray-400">I agree to the <Link to="/terms" className="text-[#0F7B8C] dark:text-[#3BA7BC] hover:underline">Terms of Service</Link> and <Link to="/privacy" className="text-[#0F7B8C] dark:text-[#3BA7BC] hover:underline">Privacy Policy</Link>.</span></label>
                            </div>
                            <div className="border-t border-gray-200 dark:border-gray-700 pt-5">
                                <label className="flex items-start gap-3 cursor-pointer"><input type="checkbox" checked={formData.isUnder18} onChange={(e) => updateField("isUnder18", e.target.checked)} className="mt-1 w-4 h-4 rounded border-gray-200 dark:border-gray-700 text-[#0F7B8C] focus:ring-[#0F7B8C]" /><span className="text-sm font-medium text-gray-900 dark:text-white">I am under 18 years old and need parental/guardian consent.</span></label>
                                {formData.isUnder18 && (
                                    <div className="mt-4 space-y-4 pl-7 border-l-2 border-amber-400 ml-2">
                                        <p className="text-sm text-amber-600 dark:text-amber-400 font-medium">Parent/Guardian Information Required</p>
                                        <div><label className="block text-sm font-medium text-gray-900 dark:text-white mb-2">Parent/Guardian Full Name *</label><input type="text" value={formData.parentName} onChange={(e) => updateField("parentName", e.target.value)} className="w-full px-4 py-3 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl text-gray-900 dark:text-white focus:outline-none focus:border-[#0F7B8C] focus:ring-2 focus:ring-[#0F7B8C]/20 transition-all" /></div>
                                        <div><label className="block text-sm font-medium text-gray-900 dark:text-white mb-2">Parent/Guardian National ID *</label><input type="text" value={formData.parentNationalId} onChange={(e) => updateField("parentNationalId", e.target.value)} className="w-full px-4 py-3 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl text-gray-900 dark:text-white focus:outline-none focus:border-[#0F7B8C] focus:ring-2 focus:ring-[#0F7B8C]/20 transition-all" /></div>
                                        <div><label className="block text-sm font-medium text-gray-900 dark:text-white mb-2">Parent/Guardian Phone</label><input type="tel" value={formData.parentPhone} onChange={(e) => updateField("parentPhone", e.target.value)} className="w-full px-4 py-3 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl text-gray-900 dark:text-white focus:outline-none focus:border-[#0F7B8C] focus:ring-2 focus:ring-[#0F7B8C]/20 transition-all" /></div>
                                    </div>
                                )}
                            </div>
                        </div>
                    ...
                    {step === 5 && (
                        <div className="space-y-5 animate-fade-in">
                            <h2 className="text-xl font-bold text-gray-900 dark:text-white flex items-center gap-2"><CheckCircle size={20} className="text-[#10B981]" /> Review & Submit</h2>
                            <p className="text-sm text-gray-600 dark:text-gray-400">Please review your information before submitting.</p>
                            <div className="space-y-3">
                                {[
                                    { label: "Personal Info", fields: [["National ID", formData.nationalId], ["Name", `${formData.firstName} ${formData.lastName}`], ["DOB", formData.dateOfBirth || "—"], ["Occupation", formData.occupation || "—"]] },
                                    { label: "Contact", fields: [["Phone", formData.phoneNumber], ["Email", formData.email || "—"], ["Region", formData.region || "—"], ["District", formData.district || "—"]] },
                                    { label: "Identity", fields: [["ID Type", formData.idType], ["Document #", formData.idNumber], ["Proof of Address", formData.proofOfAddressType], ["Stripe Verification", verificationStatus || "Not completed"]] },
                                ].map((section, i) => (
                                    <div key={i} className="bg-gray-100/30 dark:bg-gray-800/30 rounded-xl p-4">
                                        ...
                                        <div className="grid grid-cols-2 gap-1 text-sm">{section.fields.map(([l, v], j) => <div key={j} className="contents"><span className="text-gray-600 dark:text-gray-400">{l}:</span><span className="text-gray-900 dark:text-white font-medium">{v}</span></div>)}</div>
                                    </div>
                                ))}
                            </div>
                            <button onClick={handleSubmit} disabled={isSubmitting}
                                ...
                                {isSubmitting ? <><Loader2 size={20} className="animate-spin" /> Creating your account...</> : <><ShieldCheck size={20} /> Create My Account</>}
                            </button>
                        </div>
                    )}
                    ...
                    <div className="flex items-center justify-between mt-8 pt-6 border-t border-gray-200 dark:border-gray-700">
                        {step > 1 ? <button onClick={handleBack} className="flex items-center gap-2 px-5 py-2.5 border border-gray-200 dark:border-gray-700 text-gray-900 dark:text-white font-semibold rounded-xl hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors"><ArrowLeft size={16} /> Back</button> : <div />}
                        {step < TOTAL_STEPS ? <button onClick={handleNext} className="flex items-center gap-2 px-6 py-2.5 bg-[#0F7B8C] hover:bg-[#3BA7BC] text-white font-semibold rounded-xl transition-all">Continue <ArrowRight size={16} /></button> : null}
                    </div>
                </div>

                <p className="text-center text-sm text-gray-600 dark:text-gray-400 mt-6">
                    Already have an account? <Link to="/login" className="text-[#0F7B8C] dark:text-[#3BA7BC] hover:text-[#0A5D6B] dark:hover:text-[#3BA7BC]/80 font-semibold">Sign In</Link>
                </p>
            </div>
        </section>
    );
